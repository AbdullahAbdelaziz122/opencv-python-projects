...
while True:
   ...
   for i in range(len(points)):
    ...
    # Resize frame and mask before displaying
   scaled_frame = cv2.resize(frame, (640, 480))
   scaled_mask = cv2.resize(mask, (640, 480))
   cv2.imshow("Live Drawing", scaled_frame)
   cv2.imshow("mask", scaled_mask)

   scaled_paint = cv2.resize(paintWindow, (640, 480))
   cv2.imshow("paint", scaled_paint)
